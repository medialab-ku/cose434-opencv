...
protected:
	void CalibrateCamera(cv::Mat& out_cameraMatrix, cv::Mat& out_distCoeffs)
	{
		...
		for (int idx = 1; idx <= NUM_CALIB_IMAGES; idx += 1) {
		...
		double rms = cv::calibrateCamera(list_of_object_points, list_of_image_points, CALIB_IMAGE_SIZE,
